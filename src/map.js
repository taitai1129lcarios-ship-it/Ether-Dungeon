...
export class Map {
    ...
    generate() {
        ...
        do {
            ...
            // 3b. Fill Voids (User Request: 10 tile radius wall check)
            // preventing large empty spaces
            let voidRoomsAdded = 0;
            for (let y = 10; y < this.height - 10; y += 5) {
                for (let x = 10; x < this.width - 10; x += 5) {
                    if (voidRoomsAdded >= 10) break; // Limit extra rooms

                    // Check 10 radius for pure walls
                    let isVoid = true;
                    // Optimization: Check center and corners first
                    ...
                    if (isVoid) {
                        ...
                        if (placed) {
                            voidRoomsAdded++;
                            // console.log(`Filled void at ${x},${y}`);
                        }
                    }
                }
            }
    ...
    generateTraining() {
        ...
        this.tiles = [];
        this.roomGrid = [];
        this.rooms = []; // No rooms needed, but keep array to avoid errors
        ...
        console.log("Training Map Generated (40x40)");
    }
    ...
    placeRoom(config) {
        // Try random positions if not specified
        ...
        for (let i = 0; i < maxAttempts; i++) {
            ...
            if (config.x !== undefined && config.y !== undefined) {
                ...
                x = Math.floor(Math.random() * (this.width - config.w - 4)) + 2; // +2 padding for connectors
                y = Math.floor(Math.random() * (this.height - config.h - 4)) + 2;
            }
    ...
    generateConnectors(room, count) {
        ...
        // Special handling for 4x4 rooms (centered)
        if (room.w === 4 && room.h === 4) {
            possible.length = 0; // Clear randoms
            possible.push({ x: room.x + 1, y: room.y, dir: { x: 0, y: -1 }, used: false }); // Top
            possible.push({ x: room.x + 1, y: room.y + 3, dir: { x: 0, y: 1 }, used: false }); // Bottom
            possible.push({ x: room.x, y: room.y + 1, dir: { x: -1, y: 0 }, used: false }); // Left
            possible.push({ x: room.x + 3, y: room.y + 1, dir: { x: 1, y: 0 }, used: false }); // Right
            possible.sort(() => Math.random() - 0.5);
        }
    ...
    draw(ctx, camera, player, debugMode = false) {
        ...
        // --- Draw Special Features (Stairs/Portal) ---
        // Draw AFTER tile loop to prevent overdraw clipping
        for (const room of this.rooms) {
            if (room.type === 'staircase') {
                // Check visibility loosely
                if (room.x * this.tileSize > endX * this.tileSize || (room.x + room.w) * this.tileSize < startX * this.tileSize ||
                    room.y * this.tileSize > endY * this.tileSize || (room.y + room.h) * this.tileSize < startY * this.tileSize) {
                    continue;
                }
